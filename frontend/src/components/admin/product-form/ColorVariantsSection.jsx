...
export default function ColorVariantsSection({
  ...
  const handleAddVariant = () => {
    ...
    if (!newColorName.trim()) {
      setValidationError(
        "Please enter a color name (e.g. 'Space Black', 'Natural Titanium')."
      );
      return;
    }
  ...
  return (
    <div className="rounded-2xl border-2 border-slate-200 dark:border-white/15 bg-white dark:bg-gradient-to-b dark:from-[#141824] dark:via-[#0d1017] dark:to-[#080a0e] p-3.5 sm:p-6 lg:p-7 shadow-sm dark:shadow-[0_4px_25px_rgba(0,0,0,0.8),_0_0_15px_rgba(255,255,255,0.05)] space-y-4 sm:space-y-6">
        ...
        <div className="flex items-start sm:items-center gap-2.5 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-sm sm:text-lg font-heading font-bold text-slate-900 dark:text-white truncate">
              Color Finishes & Variant Inventory
            </h2>
            <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 line-clamp-2">
              Configure individual finish swatches, allocate dedicated warehouse stock per color, and set custom pricing overrides.
            </p>
          </div>
        </div>

        {/* Total Variants & Aggregate Inventory Badge */}
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[11px] sm:text-xs font-mono px-3 py-1 rounded-full bg-slate-100 dark:bg-white/[0.05] border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 font-semibold flex items-center gap-1.5 shadow-xs">
            <Boxes className="w-3.5 h-3.5 text-sky-500" />
            <span>
              {colorVariants.length} {colorVariants.length === 1 ? "Color" : "Colors"} · {totalVariantStock} Units
            </span>
          </span>
        </div>
      </div>
      ...
      {colorVariants.length > 0 && (
        <div className="p-3 sm:p-3.5 rounded-xl bg-sky-500/10 border border-sky-500/25 text-sky-700 dark:text-sky-300 text-xs font-mono flex items-center justify-between gap-2 shadow-xs">
          <div className="flex items-center gap-2 min-w-0">
            <RefreshCw className="w-4 h-4 text-sky-500 shrink-0" />
            <span className="truncate">
              Warehouse inventory is synchronized: Total stock in Section 02 will reflect <strong>{totalVariantStock} units</strong>.
            </span>
          </div>
          <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-sky-500/20 text-sky-600 dark:text-sky-300 shrink-0">
            Auto-Sync
          </span>
        </div>
      )}
      ...
      <div className="p-3.5 sm:p-5 rounded-2xl bg-slate-50 dark:bg-[#090b10]/90 border border-slate-200 dark:border-white/10 space-y-3 sm:space-y-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3">
          ...
          <div className="space-y-1">
            <label className="text-[10px] sm:text-[11px] font-mono uppercase text-slate-500 dark:text-slate-400">
              HEX Swatch
            </label>
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg border border-slate-300 dark:border-white/20 bg-transparent cursor-pointer p-0.5 shrink-0"
                title="Choose Color"
              />
              <input
                type="text"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
          ...
          <button
            ...
            className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-pink-500 hover:bg-pink-600 text-white text-xs font-mono font-bold flex items-center justify-center gap-1.5 transition-all shadow-md cursor-pointer active:scale-98"
          >
      ...
      {/* Production-Grade Color Inventory Matrix */}
      {colorVariants.length > 0 ? (
            ...
            {colorVariants.map((variant, idx) => {
              ...
              return (
                <div
                  ...
                  className={`p-3.5 sm:p-4 rounded-2xl border-2 transition-all space-y-3 shadow-xs ${
                    variant.isDefault
                      ? "border-pink-500/70 bg-pink-500/[0.04] dark:bg-pink-500/[0.08]"
                      : "border-slate-200 dark:border-white/10 bg-slate-50/70 dark:bg-[#090b10] hover:border-slate-300 dark:hover:border-white/20"
                  }`}
                >
                  ...
                  <div className="flex items-center justify-between gap-2.5">
                    <div className="flex items-center gap-2.5 min-w-0">
                      {/* Swatch Bubble */}
                      <div
                        className="w-8 h-8 rounded-full border-2 border-black/20 dark:border-white/30 shadow-xs shrink-0 flex items-center justify-center"
                        style={{ backgroundColor: variant.colorCode }}
                      >
                        {variant.isDefault && (
                          <Star className="w-3.5 h-3.5 text-white fill-white drop-shadow-xs" />
                        )}
                      </div>
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {variant.isDefault && (
                            <span className="text-[9px] px-1.5 py-0.2 rounded-full bg-pink-500 text-white font-mono uppercase font-bold shrink-0">
                              Default
                            </span>
                          )}
                        </div>
                        <p className="text-[10px] font-mono text-slate-500 dark:text-slate-400 truncate">
                          {variant.colorCode} {variant.sku ? `· SKU: ${variant.sku}` : ""}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-white dark:bg-[#12141c] border border-slate-200 dark:border-white/10 flex flex-wrap items-center justify-between gap-2.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        disabled={stockNum <= 0}
                        className="w-7 h-7 rounded-lg bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 text-slate-700 dark:text-slate-200 font-bold flex items-center justify-center transition-all disabled:opacity-30 cursor-pointer"
                      ...
                      <input
                        ...
                        className="w-14 px-1.5 py-1 text-center font-mono font-bold text-xs bg-slate-50 dark:bg-[#090b10] border border-slate-200 dark:border-white/15 rounded-md text-slate-900 dark:text-white outline-none focus:border-pink-500"
                      />

                      <button
                        type="button"
                        onClick={() => handleIncrementStock(idx)}
                        className="w-7 h-7 rounded-lg bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 text-slate-700 dark:text-slate-200 font-bold flex items-center justify-center transition-all cursor-pointer"
                  ...
                  <div className="flex items-center justify-between text-[11px] font-mono text-slate-500 dark:text-slate-400 pt-0.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <input
                        ...
                        onChange={(e) => handleUpdatePriceOverride(idx, e.target.value)}
                        placeholder="Default MRP"
                        className="w-24 px-1.5 py-0.5 text-right font-mono text-[11px] font-semibold bg-white dark:bg-[#12141c] border border-slate-200 dark:border-white/10 rounded-md text-slate-900 dark:text-white outline-none focus:border-pink-500"
        ...
        <div className="p-4 rounded-xl border border-dashed border-slate-300 dark:border-white/10 text-center text-xs font-mono text-slate-400 dark:text-slate-500 space-y-1">
          <p className="font-semibold text-slate-600 dark:text-slate-400">
            No color finishes configured yet.
          </p>
          <p className="text-[11px]">
            Add a finish above to track inventory and swatches individually for each hardware variant.
          </p>
        </div>
      )}
    </div>
