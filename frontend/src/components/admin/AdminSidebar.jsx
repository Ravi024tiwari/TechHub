...
export default function AdminSidebar({ isMobileOpen, setIsMobileOpen, isCollapsed, setIsCollapsed }) {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-50 flex flex-col bg-[#08090a] border-r border-white/10 transition-all duration-300 ease-in-out
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-5 border-b border-white/10 shrink-0">
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-white to-slate-400 p-[1px] shadow-lg shadow-white/5">
              <div className="w-full h-full rounded-[11px] bg-[#0e0f13] flex items-center justify-center">
            ...
            {!isCollapsed && (
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  <span className="font-heading font-bold text-white text-base tracking-tight">TechHub</span>
                  <span className="px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold uppercase tracking-wider bg-white/10 text-white/90 border border-white/15">
                    Admin
                  </span>
                </div>
                <span className="text-[11px] font-mono text-slate-400 -mt-0.5">Control Center</span>
              </div>
            )}
          </NavLink>
                    ...
                    >
                      ...
                      {!isCollapsed && (
                        <>
                          ...
                          {item.badge && (
                            <span className="px-1.5 py-0.5 text-[10px] font-mono uppercase tracking-wide rounded-full bg-amber-500/20 text-amber-300 border border-amber-500/30">
                              {item.badge}
                            </span>
                          )}
                          <ChevronRight
                            className={`w-3.5 h-3.5 text-slate-600 transition-transform duration-200 opacity-0 group-hover:opacity-100 ${
                              isActive ? "opacity-100 text-black/50" : ""
                            }`}
                          />
                        </>
                      )}
        ...
        <div className="p-3.5 border-t border-white/10 shrink-0">
          <NavLink
            ...
            className={`flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-medium text-slate-300 bg-white/[0.03] hover:bg-white/[0.08] hover:text-white border border-white/5 transition-all ${
              isCollapsed ? "justify-center px-0" : ""
            }`}
        ...
        <div className="p-3.5 bg-[#0b0c10] border-t border-white/10 shrink-0 space-y-3">
          {/* User Profile Card */}
          ...
          <button
            ...
            className={`w-full flex items-center gap-2.5 py-2 px-3 rounded-xl text-xs font-mono font-semibold transition-all duration-200 border cursor-pointer ${
              isCollapsed
                ? "justify-center px-0 text-rose-400 hover:bg-rose-500/20 border-rose-500/20 hover:border-rose-500/40"
                : "bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 hover:text-rose-200 border-rose-500/25 hover:border-rose-500/40 shadow-sm shadow-rose-950/20 hover:shadow-rose-900/30"
            }`}
