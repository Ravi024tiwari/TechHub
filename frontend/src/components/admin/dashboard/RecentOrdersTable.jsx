...
export default function RecentOrdersTable({
  ...
  const getStatusBadge = (status = "") => {
    ...
    switch (s) {
      ...
      case "OUT_FOR_DELIVERY":
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-mono font-medium bg-sky-500/10 text-sky-400 border border-sky-500/20">
      ...
      case "PLACED":
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-mono font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20">
      ...
      default:
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-mono font-medium bg-white/10 text-slate-300">
            {status}
          </span>
        );
    }
  ...
  // Fallback demo orders if brand new database without orders
  const displayOrders =
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-[11px] text-slate-400">
            Real-time customer transactions and payment states
          </p>
        </div>
        <Link
          to="/admin/orders"
          ...
          <tbody className="divide-y divide-white/5">
            {displayOrders.map((order) => {
              ...
              return (
                ...
                >
                  <td className="py-3 px-5 sm:px-6 font-mono font-semibold text-sky-400">
                    <Link
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-slate-800 text-[10px] font-bold text-slate-300 flex items-center justify-center border border-white/10 shrink-0">
                        {order.user?.name?.charAt(0) || "U"}
                      </div>
                      <span className="font-medium text-white truncate max-w-[110px] sm:max-w-[140px]">
                        {order.user?.name || "Customer"}
                      </span>
                    </div>
                  </td>
                  <td className="py-3 px-3 text-slate-300 truncate max-w-[140px] hidden md:table-cell">
                    {order.productName || "Electronics Item"}
                  </td>
                  ...
                  <td className="py-3 px-5 sm:px-6 text-right font-mono text-slate-500 text-[11px] whitespace-nowrap">
